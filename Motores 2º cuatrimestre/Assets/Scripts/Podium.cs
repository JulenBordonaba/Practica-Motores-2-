...
public class Podium : MonoBehaviour
{
    ...
    void Start()
    {
        for (int i = 0; i < G.positions.Count; i++)
        {
            if (G.positions[i] != 0)
            {
                StartCoroutine(PonerEnPodio(i, G.positions[i]));
            }
    ...
    public IEnumerator PonerEnPodio(int player, int position)
    {
        ...
        do
        {
            podiums[player].transform.position = Vector3.Lerp(startPosition, startPosition + new Vector3(0, (4 - position) * 1f, 0), time);
